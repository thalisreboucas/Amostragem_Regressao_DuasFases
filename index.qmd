...
$V(\hat{t}^{(2)}_{y, GREG} | Z) = V \left( \sum_{i \in S(2)} w_i^{(1)} w_i^{(2)} d_i^{(1)} \middle| Z \right),$

onde $d_i^{(1)} = y_i - x_i^T \hat{B}^{(1)} $e $\hat{B}^{(1)} = \left( \sum_{i \in S(1)} \left( \frac{w_i^{(1)}}{\sigma^2_i} \right) x_i x_i^T \right)^{-1} \sum_{i \in S(1)} \left( \frac{w_i^{(1)}}{\sigma^2_i} \right) x_i y_i$.
...
com $e_i = y_i - x_i \hat{B}^{(2)}$ substituído pelos valores desconhecidos $d_i^{(1)}$. Se o plano amostral da fase I for uma SRS (amostra aleatória simples) de $n^{(1)}$ unidades, 

$\hat{V}(\hat{t}^{(1)}_y) = N^2 \left( 1 - \frac{n^{(1)}}{N} \right) \frac{\hat{S}^2_y}{n^{(1)}},$

com

$\hat{S}^2_y = \frac{1}{n^{(2)} - 1} \sum_{i \in S(2)} (y_i - \bar{y}^{(2)})^2.$
