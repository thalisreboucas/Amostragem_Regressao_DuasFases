...
- Na primeira fase, selecione uma amostra relativamente grande de elementos,$s_1$,coletando informações de baixo custo sobre uma ou mais variáveis auxiliares.

- Com o auxílio das informações auxiliares coletadas na primeira fase, selecionamos uma amostra de segunda fase,$s_2$, a partir de $s_1$,utilizando o delineamento $p(.|s_1)$, com isso $s_2$ uma subamostra. A variável de estudo $y$ é então observada para os elementos na amostra da segunda fase.
...
$$\pi_{ak} = \sum_{k \in s1}p_a(s1) ,\ \pi_{akl} = \sum_{k \in s1}p_a(s1) , $$
...
$$\pi_{k|s1} = \sum_{k \in s2}p(s2|s1) ,\ \pi_{kl|s1} = \sum_{kl \in s1}p_a(s2|s1) , $$
...
# Estimador regressão generalizada (GREG) em duas fases

## Estimador regressão generalizada (GREG) em duas fases
**Estimador GREG para duas fases**

Os estimadores GREG são assintoticamente não viesados em relação ao plano amostral (Särndal, et al. 1992), independentemente da escolha dos regressores. A menos que a amostra seja pequena, o viés pode ser desprezado. É evidente que o estimador GREG será mais eficiente — no sentido de ter menor variância — do que o estimador de Horvitz-Thompson. Como resultado, o estimador GREG geralmente terá um desempenho superior ao estimador de Horvitz-Thompson em termos de erro quadrático médio (a soma da variância e do quadrado do viés).

## Estimador regressão generalizada (GREG) em duas fases
**Estimador GREG para duas fases**

$$\hat t^{(2)}_{yGREG} = \hat t^{(2)}_y+(\hat t^{(1)}_x-\hat t^{(2)}_x)^T\hat \beta^{(2)}$$
sendo $\hat \beta^{(2)}$,

$$\hat \beta^{(2)}\left(\sum_{i \in S^{(2)}} w^{(1)}_i w^{(2)}_i \dfrac{1}{\sigma^2_i}x_ix_i^T\right )^{-1}\sum_{i \in S^{(2)}}w_i^{(1)}w^{(2)}_i\dfrac{1}{\sigma^2_i}x_iy_i$$

## Estimador regressão generalizada (GREG) em duas fases
**Estimador GREG para duas fases**


- $S(2)$: Representa a amostra da fase II (subamostra da amostra da fase I).

- $w_i^{(1)}$: Peso da amostra da fase I (inverso da probabilidade de seleção na fase I).

- $w_i^{(2)}$: Peso da amostra da fase II (inverso da probabilidade de seleção na fase II).

- $g_i$: Fator de ajuste baseado na relação entre y e as variáveis auxiliares x. Esse fator corrige o viés no estimador inicial usando as informações auxiliares.

## Estimador regressão generalizada (GREG) em duas fases
**Estimador GREG para duas fases**

O estimador $\hat t^{(2)}_{yGREG}$ pode ser escrito usando uma modificação dos pesos.

A equação para $g_i$ é dada por:

$$
g_i = 1 + (\hat{t}^{(1)}_x - \hat{t}^{(2)}_x)^T \left(\sum_{i \in S^{(2)}} w_i^{(1)} w_i^{(2)} \frac{1}{\sigma^2_i} x_i x_i^T \right)^{-1} \frac{1}{\sigma^2_i} x_i
$$

Então, o estimador GREG é expresso como:

$$
\hat{t}^{(2)}_{y, GREG} = \sum_{i \in S^{(2)}} w_i^{(1)} w_i^{(2)} g_i y_i
$$

## Estimador regressão generalizada (GREG) em duas fases
**Estimador GREG para duas fases**

Por linearização, a variância condicional do estimador GREG pode ser expressa como:
$$
V\left(\hat{t}^{(2)}_{y, GREG} \middle| Z \right) = V \left( \sum_{i \in S^{(2)}} w_i^{(1)} w_i^{(2)} d_i^{(1)} \middle| Z \right),$$
...
- $d_i^{(1)} = y_i - x_i^T \hat{\beta}^{(1)}$,

- $\hat{\beta}^{(1)} = \left( \sum_{i \in S^{(1)}} \left( \frac{w_i^{(1)}}{\sigma^2_i} \right) x_i x_i^T \right)^{-1} \sum_{i \in S^{(1)}} \left( \frac{w_i^{(1)}}{\sigma^2_i} \right) x_i y_i$.

Aqui, $\hat{\beta}^{(1)}$ é o vetor estimado dos coeficientes de regressão baseados na primeira fase da amostra, e $d_i^{(1)}$ é o resíduo da fase I (diferença entre a variável de interesse $y_i$ e a predição baseada nas variáveis auxiliares $x_i$).

## Estimador regressão generalizada (GREG) em duas fases
**Estimador GREG para duas fases**

A variância condicional no segundo termo é não viesada para sua expectativa, então $E \left( V(\hat{t}^{(2)}_{y, GREG} | Z) \right)$ pode ser estimada por:

$$
\hat{V}(\hat{t}^{(2)}_{y, GREG} | Z) = \hat{V} \left( \sum_{i \in S^{(2)}} w_i^{(1)} w_i^{(2)} e_i \middle| Z \right),
$$

com $e_i = y_i - x_i \hat{B}^{(2)}$ substituindo os valores desconhecidos $d_i^{(1)}$. Aqui, $e_i$ é o resíduo da fase II, usando os coeficientes de regressão $\hat{B}^{(2)}$ estimados a partir dos dados da fase II.

## Estimador regressão generalizada (GREG) em duas fases
**Estimador GREG para duas fases**

Se o plano amostral da fase I for uma Amostra Aleatória Simples (AAs) de $n^{(1)}$ unidades, a variância do estimador da fase I pode ser estimada por:
$$
\hat{V}(\hat{t}^{(1)}_y) = N^2 \left( 1 - \frac{n^{(1)}}{N} \right) \frac{\hat{S}^2_y}{n^{(1)}},
$$
onde:

- $\hat{S}^2_y = \frac{1}{n^{(2)} - 1} \sum_{i \in S^{(2)}} (y_i - \bar{y}^{(2)})^2$ é a estimativa da variância amostral da variável de interesse $y$ na fase II, $\bar{y}^{(2)}$ é a média de $y$ na amostra da fase II.
...
**Fase 1:**
Amostragem Aleatória Simples Sem Reposição (AASs): Selecione aleatoriamente 20 unidades (indivíduos) do bloco, sem permitir repetições. Anote apenas a idade de cada indivíduo selecionado nesta fase.
...
```
```{r}
print("Amostra seleciona da de s1") ;s1 ; 
...
Modelo proposto será:
$$\hat{y}_{reg}= 389,141 -4,56(67,692-65.218)$$
$$MSE=\left(\dfrac{1}{20}-\dfrac{1}{50}\right)3246,794+\left(\dfrac{1}{10}-\dfrac{1}{20}\right)3246,794(1-0,4989^2)$$

```{r}
print("Meu y estimado de regressão foi "); ybrd
print("Meu MSE foi e ");Mse
print("Meu CVE foi e ");round(sqrt(Mse)/Mse,4)
```
...
```{r}
print("limite superior e Limite inferior foram:");UL ;LL
```
Comparando com agora com o valor populacional

```{r}
print('Média de sono populacional');mean(y);print('Variância de sono populacional');var(y)
```
...
- Raosaheb Latpate, Jayant Kshirsagar , Advanced Sampling Methods (2021).
