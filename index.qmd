...
- $d_i^{(1)} = y_i - x_i^T \hat{B}^{(1)}$,

- $\hat{B}^{(1)} = \left( \sum_{i \in S^{(1)}} \left( \frac{w_i^{(1)}}{\sigma^2_i} \right) x_i x_i^T \right)^{-1} \sum_{i \in S^{(1)}} \left( \frac{w_i^{(1)}}{\sigma^2_i} \right) x_i y_i$.

Aqui, $\hat{B}^{(1)}$ é o vetor estimado dos coeficientes de regressão baseados na primeira fase da amostra, e $d_i^{(1)}$ é o resíduo da fase I (diferença entre a variável de interesse $y_i$ e a predição baseada nas variáveis auxiliares $x_i$).
