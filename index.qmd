...
$$ \hat \beta_1 = \frac{\sum_{i=1}^{N} (x_i − \bar{x}_U)(y_i − \bar{y}_U)}{\sum_{i=1}^{N} (x_i − \bar{x}_U)^2} = R \frac{S_y}{S_x}, $$
$$\hat \beta_0 = \bar{y}_U − \hat\beta_1 \bar{x}_U$$.
...
$$
\hat{\beta} = (\sum_Ux_kx_k'/\sigma^2_k\pi^*_k)^{-1}\sum_Ux_ky_k/\sigma^2_k \pi^*_k).
$$
